...
class MainMenu:
    ...
    def setup_styles(self):
        ...
        self.button_style.configure('Easy.TButton', font=('Helvetica', 12, 'bold'), foreground='white', background=self.COLORS['button_easy'], borderwidth=0, relief='flat', padding=15)
        self.button_style.configure('Intermediate.TButton', font=('Helvetica', 12, 'bold'), foreground='white', background=self.COLORS['button_intermediate'], borderwidth=0, relief='flat', padding=15)
        self.button_style.configure('Expert.TButton', font=('Helvetica', 12, 'bold'), foreground='white', background=self.COLORS['button_expert'], borderwidth=0, relief='flat', padding=15)
        self.button_style.configure('Custom.TButton', font=('Helvetica', 10, 'bold'), foreground='white', background=self.COLORS['button_custom'], relief='flat', padding=10)
        self.button_style.configure('Highscore.TButton', font=('Helvetica', 10, 'bold'), foreground='white', background=self.COLORS['accent'], relief='flat', padding=10)
        self.button_style.configure('Analytics.TButton', font=('Helvetica', 10, 'bold'), foreground='white', background=self.COLORS['button_secondary'], relief='flat', padding=10)
        self.button_style.configure('Exit.TButton', font=('Helvetica', 10, 'bold'), foreground='white', background='#c0392b', relief='flat', padding=10)
        self.button_style.configure('DialogButton.TButton', font=('Helvetica', 11, 'bold'), foreground='white', background=self.COLORS['success'], relief='flat', padding=10)
        # Hover map
        for style_name, color in [
            ('Easy', self.COLORS['button_easy']), ('Intermediate', self.COLORS['button_intermediate']), ('Expert', self.COLORS['button_expert']),
            ('Custom', self.COLORS['button_custom']), ('Highscore', self.COLORS['accent']), ('Analytics', self.COLORS['button_secondary']), ('Exit', '#c0392b'),
    ...
    def create_widgets(self):
        ...
        title = tk.Label(
            title_frame, text="💣 MINESWEEPER", font=("Helvetica", 36, "bold"),
            fg=self.COLORS['primary'], bg=self.COLORS['bg']
        )
        title.pack()
        subtitle = tk.Label(
            title_frame, text="Select Your Challenge",
            font=("Helvetica", 14),
        ...
        ttk.Button(
            options_frame, text="⚙️ Custom", command=self.custom_game, style='Custom.TButton'
        ).grid(row=0, column=0, padx=5, pady=5, sticky='ew')
        ttk.Button(
            options_frame, text="🏆 Highscores", command=self.view_highscores, style='Highscore.TButton'
        ).grid(row=0, column=1, padx=5, pady=5, sticky='ew')
        ttk.Button(
            options_frame, text="📊 Analytics", command=self.generate_analytics, style='Analytics.TButton'
        ).grid(row=1, column=0, padx=5, pady=5, sticky='ew')
        ttk.Button(
            options_frame, text="❌ Exit", command=self.root.quit, style='Exit.TButton'
    ...
    def custom_game(self):
        ...
        def start_custom():
            try:
                ...
                if mines < 1:
                    messagebox.showerror("Invalid Input", "You must have at least 1 mine!")
                    return
                if mines >= total_cells:
                    messagebox.showerror(
                        "Invalid Input", 
                        f"Too many mines!\n\nBoard has {total_cells} cells.\nYou entered {mines} mines.\n\nMines must be less than total cells.\nMaximum: {total_cells - 1}"
                    )
                    return
                if mines > max_mines:
                    messagebox.showerror(
                        "Invalid Input", 
                        f"Too many mines for this board size!\n\nBoard: {rows}×{cols} ({total_cells} cells)\nMines: {mines}\nRecommended maximum: {max_mines}\n\nTip: Mines should be 10-20% of total cells\nfor a balanced game."
                    )
                    return
                ...
                if mine_percentage > 60:
                    response = messagebox.askyesno(
                        ...
                        f"Your board has {mine_percentage:.1f}% mines!\n\nThis will make the game extremely difficult.\nRecommended: 10-25% mine density.\n\nContinue anyway?",
                        icon='warning'
                    )
                    if not response:
                        return
